...
4. Treat the evidence bundle as complete. If it cannot support the required coverage, return `coverage-gap` so source configuration can be improved or a reviewed manual lead can be added deliberately. For each citation, use `landingUrl` as the human-readable `url` when supplied, preserve the release-specific `canonicalUrl` as `evidenceUrl`, and preserve its identifier as `releaseId`.
