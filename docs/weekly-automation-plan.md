...
- Candidates are near-duplicate clustered so one story from several outlets collapses to a single ranked representative with the others attached as related links. Dated releases in the same recurring dataset are consolidated separately and never counted as corroborating publishers. Candidates are ordered by a deterministic relevance score (recency, source authority weight, news/dataset class, keyword density, cross-outlet corroboration, and under-represented-coverage adjustments). Each new candidate stores the exact score inputs and contributions. The collection pull request renders those details with all configured source diagnostics before Codex is allowed to draft; the score is queue ordering, not a confidence rating.
...
- Before any Codex pass, the source-review pull request includes a deterministic audit: all configured adapters and their health/rejection counts, all ranked candidates, and each score's factor-by-factor arithmetic. Candidate JSON retains the structured score breakdown for reproducibility; it is not treated as evidence or factual confidence.
