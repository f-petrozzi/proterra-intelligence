...
## Deferred editorial source-mix correction

On 2026-08-18, the deterministic Issue 02 rerun was rolled back to the earlier approved, news-like report while all automation and security changes were retained. The deterministic queue contained only five recurring USDA datasets or market reports, so it was suitable as evidence and market-pulse material but not as the complete discovery universe for a news-summary reel.

Before the next automated editorial run, separate discovery-led developments from supporting evidence. Readiness should require enough genuinely news-led candidates; routine dataset releases should normally support stories or populate the dashboard, with at most an exceptional material release promoted into the reel. A coverage override must not waive that newsworthiness requirement. Reader-facing citations should prefer a human-readable report landing or synopsis page; the exact PDF, dated release, or API response should remain attached separately as the auditable evidence URL rather than becoming the primary link. Published-email links should eventually use the stable report permalink while immutable deployment URLs remain stored for audit. This work is intentionally deferred and was not implemented as part of the Issue 02 restoration.
