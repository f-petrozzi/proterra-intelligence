...
- Candidates are near-duplicate clustered so one story from several outlets collapses to a single ranked representative with the others attached as related links, and are ordered by a deterministic relevance score (recency, source authority weight, keyword density, cross-outlet corroboration, and an under-represented-coverage boost).
