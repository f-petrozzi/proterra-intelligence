...
async function selectField(target){document.querySelectorAll(selectable).forEach((element)=>element.classList.remove('selected'));target.classList.add('selected');const fieldValue=normalize(target.textContent||'');const selection=window.getSelection();let selectedText='';if(selection&&selection.rangeCount>0&&target.contains(selection.anchorNode)&&target.contains(selection.focusNode))selectedText=normalize(selection.toString()).slice(0,1000);const offset=selectedText?fieldValue.indexOf(selectedText):-1;selected={anchorKey:target.dataset.reviewAnchor,storyReviewId:target.dataset.storyReviewId,fieldPath:target.dataset.reviewFieldPath,anchorLabel:target.dataset.reviewLabel,selectedText,contextBefore:offset>=0?fieldValue.slice(Math.max(0,offset-120),offset):'',contextAfter:offset>=0?fieldValue.slice(offset+selectedText.length,offset+selectedText.length+120):'',fieldValueHash:await hash(fieldValue)};byId('selection-label').textContent=selected.anchorLabel;byId('selection-quote').textContent=selected.selectedText||selected.fieldPath;byId('comment-body').disabled=false;byId('add-comment').disabled=false;byId('comment-body').focus()}
